...
const { host, token, projectId, outDir, enumKeys = {} } = require(`${cwd}/package.json`).yapi
...
function template ({ menu, title, name, list }) {
  ...
  const code = ({ description, key, type, required, isObject }) => {
    ...
    if (Array.isArray(type)) {
       ...
       * 列表子项
       */
      ${camelcase(key)}${flag}: I${name}Item${isObject ? '' : '[]'}
        `
    }
   ...
   * ${description || ''}
   */
  ${camelcase(key)}${flag}: ${enumKeys[key] || type}
    `
  }
...
function transform (body, requiredNames) {
  ...
  Object.keys(properties).forEach((key) => {
    ...
    if (type === 'object') {
      properties[key].type = transform(properties[key].properties)
      properties[key].isObject = true
    }
    ...
    tryer({
      ...
      exector: function (index) {
        ...
        if (Array.isArray(data)) {
          const list = data.map((o) => ({ type: o.type === 'text' ? 'string' : 'any', description: o.desc, key: o.name, required: o.required }))
